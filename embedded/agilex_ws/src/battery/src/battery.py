...
def publish_battery_percentage():
    ...
    while not rospy.is_shutdown():
        ...
        # Check if voltage is within the expected range
        if min_voltage <= voltage <= max_voltage:
        # Convert voltage to percentage
            battery_percentage = ((voltage - min_voltage) / (max_voltage - min_voltage)) * 100.0
        else:
        # Handle the case where voltage is out of range
            rospy.logwarn("Battery voltage out of range. Skipping calculation.")
